...
public class CommentService {
  ...
  public Comment create(long postId, long userId, String content) {
    try {
      ...
      throw new ApiException(ErrorCode.NOT_FOUND_RESOURCE,"게시글이 작성에 실패하였습니다.");
    }
  }
  ...
  public Comment update(long id, String content) {
    int updated = commentRepository.updateContent(id, content);
    if (updated == 0) {
      throw new ApiException(ErrorCode.NOT_FOUND_RESOURCE, "존재하지 않는 댓글입니다. id=" + id);
    }
    return CommentMapper.toDomain(commentRepository.findById(id));
  }
